...
import { MAIN_API, ROUTE_GET_PRODUCTS, ROUTE_GET_TRANSACTIONS, ROUTE_BUY_CRYPTO, ROUTE_RECHARGE_WALLET } from '../config/api_routes'
...
export const sendOffer = async (data) => {
    return await fetch(MAIN_API.length !== 0 ?  
        MAIN_API.concat(ROUTE_BUY_CRYPTO) : 'https://reqres.in/api/unknown/23', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
export const getTransactions = async (id) => {
    return await fetch(MAIN_API.length !== 0 ? 
        MAIN_API.concat(ROUTE_GET_TRANSACTIONS).concat(id) : `https://reqres.in/api/unknown/23`, {
        method: 'GET',
        redirect: "follow",
        headers: {
            'Content-Type': 'application/json',
...
export const rechargeWallet = async (id, amount) => {
    return await fetch(MAIN_API.length !== 0 ? 
        MAIN_API.concat(ROUTE_RECHARGE_WALLET).concat(id) : `https://reqres.in/api/unknown/23`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json',
        },
